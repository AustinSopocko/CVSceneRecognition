...
- **Clustering (KMeans):**
    ...
    - The `n_init` parameter was set to $5$, meaning that the KMeans algorithm runs 5 times with different centroid seeds and the best run is used. 
...
- **Optimising:** We compare this to the existing best training accuracy and/or re-run KMeans for a different k value (we test for k values in the range $[475,525]$ in increments of 5) until all values in range have been tested.
- **Output:** We retrieve the optimal k value and use this k value on the testing data provided. 

The best training accuracy was achieved for $k=515$, with a training accuracy of 89.07%. We use this on the testing data to produce run2.txt. 